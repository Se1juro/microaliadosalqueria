...
productsServices.getProducts = async (viewAllProducts) => {
  let res;
  if (!viewAllProducts) {
...
productsServices.deleteProduct = async (id,fn) => {
  Swal.fire({
    ...
    if (result.value) {
      try {
        const res = await axios.delete('http://localhost:4000/productos/' + id, {
          headers: {
            Authorization: 'Bearer ' + state.token,
          },
        });
...
productsServices.updateProduct = async (codigoReferencia, newProduct) => {
  try {
    await axios
        .put(
            'http://localhost:4000/productos/' + codigoReferencia,
            newProduct,
            {
              headers: {
                Authorization: 'Bearer ' + state.token,
